...
- **QR はサーバーで生成して画像として渡す**（`qrcode-svg`）。SVG の中に URL は現れない。
  ただしコピー用に URL も返すので、手元で開いた画面では JS と DOM がトークンを持つ
- 起動時に読んだ IPv4 を使うので、**DHCP でアドレスが変わったら monitor を再起動する**
  （古い IP を指した QR を出し続けるため）
...
ui/                  React + Vite + TypeScript + Tailwind v4（web/ と同じデザイントークン）
  ...
  src/three/         立体表示。Voxels(キャラ) / Capitol(議事堂 1 棟) / World(空間) / blueprint(寸法)
```

表示は「平面 / 立体 / 空間」の 3 つをヘッダーで切り替える（`src/render3d.ts`・localStorage 保存）。
平面が既定で、選ぶまで three.js は読み込まない。空間はセッション 1 つを議事堂 1 棟として建て、
頂上に親エージェント・2 段目にサブエージェントを立たせ、状態を建物の光り方で表す。
