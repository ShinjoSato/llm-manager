...
import { boundsOf, centerOf, voxelize, type Voxel } from "../pixel/voxelize.js";
...
function Voxels({ voxels }: { voxels: Voxel[] }) {
 ...
 */
export default function VoxelArt({
  sprite,
  palette,
  scale = 4,
  className = "",
  ...
  const shape = useMemo(() => {
    ...
    const content = boundsOf(voxels);
    const fit = fitScale(frame, projectedSize(content.width, content.height, DEPTH, ROT_X, ROT_Y));
    return { voxels, frame, fit, center: centerOf(voxels) };
  }, [sprite, palette]);
  ...
  return (
    ...
    >
      <View style={{ width: "100%", height: "100%" }}>
        ...
        <group scale={scale * shape.fit} rotation-x={ROT_X}>
          <group rotation-y={ROT_Y}>
            <group position={[-shape.center.x, -shape.center.y, 0]}>
